...
function updateValue() {
  document.getElementById("autoCompleteDiv").innerHTML = "";
  resultArr = [];
  searchInput = input.value;
  if(searchInput!=="")
  for (country of allCountries) {
    if (country.toLowerCase().startsWith(searchInput.toLowerCase())) resultArr.push(country);
  }

  for (country of resultArr) {
    let button = document.createElement("input");
    button.setAttribute("type", "button");

    button.value = country;
        button.onclick = function () {
      document.querySelector("#countryInput").value = button.value;
      console.log(document.querySelector("#countryInput").value);
      getCountryInfo();

    };
    document.getElementById("autoCompleteDiv").appendChild(button);
